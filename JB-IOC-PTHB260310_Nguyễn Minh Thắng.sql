--1. Tạo bảng (10 điểm) Tạo 4 bảng Customer , Room , Booking , Payment với cấu trúc và kiểu dữ liệu hợp lý. Đảm bảo có các khóa chính (PK) và khóa ngoại (FK) để liên kết các bảng.
...
select c.customer_id, c.customer_full_name, count(c.customer_id) as "số lượng phòng đã đặt"
from customer c
join booking b on c.customer_id=b.customer_id
group by c.customer_id, total_amount
having sum(total_amount)>1000 and count(c.customer_id)>=2
...
as $$
begin 
	insert into Payment(booking_id, payment_method, payment_amount, p_payment_date) 
	values
